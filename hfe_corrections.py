...
def a15p1_1_cleanup(data):
	...
	"""
	m,p,s='a15','p1',1
	n = np.array(data[m][p][s]['dT']).shape[0]

	# default corrected values to Hills' missing data flag.

	data[m][p][s]['dT_corr']=pd.Series(
			-9999*np.ones(n,dtype=np.float32),index=data[m][p][s].index)
	...
	ix=(np.bitwise_and(data[m][p][s]['flags'].values,0b1)==0)
	dT=data[m][p][s]['dT'].loc[ix].values
	f=data[m][p][s]['flags'].loc[ix].values
	...
	f[[1195,1756,1849,12020,18727,19571,23648,26106,26124,26837,26858,27576]]+=0b100

	# Flag extreme outliers in T.
	f[[1195,12020,16031,18286,20656,22187,26858,27576,27791,]]+=0b1000

	data[m][p][s].loc[ix,'dT_corr']=dT
	data[m][p][s].loc[ix,'flags']=f
...
def a15_1975p1_1_cleanup(data):
	...
	f[[3238,5797,7668,13857,13862,13864,14644,14648]]+=0b100

	# Flag extreme outliers in T.

	f[[7668,8807,13862,13864,14644,14648,15240]]+=0b1000
...
def a15p1_2_cleanup(data):
	...
	# Flag extreme outliers in dT.
	f[[3128,6161,6938,7041,12119,16132,18528,18891,19227,19262,23074,23707,23740,
								23758,23805,23815,26369,27693]]+=0b100

	# Flag extreme outliers in T.
	f[[2512,2527,7041,12119,16132,18420,18981,23740,23758,23815,26369,
											27613,28173]]+=0b1000
...
# we are after all simply discarding any rows marked with outliers in sets past 'clean'.
	...
	f[[2474,2679,2713,6128,9858,13109,13112,14120]]+=0b100

	# Flag extreme outliers in T.
	f[[2474,2679,6128,9858,9859,13109,13112,14120]]+=0b1000
...
def a15p1_3_cleanup(data):
	...
	f[[4350,19205,19235,19524,20030,23455]]+=0b100000

	# Flag extreme outliers in TC1.
	
	f[[4350,10908,15224,16109,19205,19235,19524,19661,20030,23455,28238]]+=0b1000000
	
	# Flag extreme outliers in TC2.
	
	f[[4350,7268,10908,15224,16220,19205,19235,19524,19661,20030,23455,28238]]+=0b10000000
	
	# Flag extreme outliers in TC3.
	
	f[[4350,10908,15224,16015,19205,19235,19524,19661,20030,23455,28238]]+=0b100000000
	
	# Flag extreme outliers in TC4.

	# NOTE: relatively noisy. only very dramatic points flagged.

	f[[4350,10908,15224,19205,19235,19524,19661,20030,23455,26731,28238]]+=0b1000000000
...
def a15p1_4_cleanup(data):
	...
	m,p,s='a15','p1',4
	n = np.array(data[m][p][s]['T']).shape[0]
...
def a15p2_1_cleanup(data):
	...
	f[2441:2443]+=100000000000
	f[4790]+=100000000000
	f[7334]+=100000000000
	f[8439]+=100000000000
	f[11102]+=100000000000
	f[19206]+=100000000000
	f[19620]+=100000000000
	f[22589:22591]+=100000000000    
	f[2441:2443]+=100000000000
	f[4790]+=100000000000
	f[7334]+=100000000000
	f[8439]+=100000000000
	...
	f[[2267,4282,5460,15561,19124,19127,19296,19923,22401,23815,23458,24046,]]+=0b100

	f[[2267,4282,4583,15561,23183]]+=0b1000
...
def a15_1975p2_1_cleanup(data):
	...
	# flag extreme outliers in dT.
	f[[4634,13088,13846,13849,14224,14628,14630,14631,17317]]+=0b100

	# Flag extreme outliers in T.
	f[[13848,14630,17317]]+=0b1000
...
def a15p2_2_cleanup(data):
	...
	dT[97:98]=2*(-2./2**11)-dT[97:98] # Ambiguous.
	f[97:98]+=0b10
	...
	# flag extreme outliers in dT.
	f[[7044,11635,16118,18846,19295,19530,19541,19881,21552,21814,23342,26473]]+=0b100

	# Flag extreme outliers in T.
	f[[2503,11635,16118,18846,19530,19541,21814,23342,26473]]+=0b1000
...
def a15_1975p2_2_cleanup(data):
	...
	# Flag extreme outliers in dT.
	f[[6618,7439,9336,10171,12601,13047,13079,13823,13824,13825,14601,14716]]+=0b100

	# Flag extreme outliers in T.
	f[[3672,7439,13047,13079,13823,13824,13825]]+=0b1000
...
def a15p2_3_cleanup(data):
	...
	f[[4341,7265,16170,19195,19225,19845,19964,19968,19980,20025,22561,23028,23749,25047,
	25804,27526,28324]]+=0b1000000

	# Flag extreme outliers in TC2.

	f[[4341,16170,19195,19225,19672,19845,19964,19968,19980,20025,22056,22561,23028,23749,
	25804,28324]]+=0b10000000

	# Flag extreme outliers in TC3.
	
	f[[4341,12708,16170,19195,19225,19845,19964,19968,19980,20025,22561,23028,23749,25047,
	25804,28324]]+=0b100000000

	# Flag extreme outliers in TC4.

	f[[4341,16170,19195,19225,19845,19964,19968,19980,20025,22561,23028,23749,25047,25804,
	28324]]+=0b1000000000
...
def a15p2_5_cleanup(data):
	...
	# Flag extreme outliers in dT.
	f[[221,231,255,475,]]+=0b100

	# Flag extreme outliers in T.
	f[[221,231,255,422,475,]]+=0b1000
...
def a17p1_1_cleanup(data):
	...
	# Flag extreme outliers in dT.
	f[[2562,3905,7026,7525,11026,11243,11910,12015,12704,13861,13865,13953,13956,14096,
	14400,15933,16921,19990,19996,20205,20208,20326,]]+=0b100

	# Flag extreme outliers in T.
	f[[3905,6102,7525,11016,11243,11910,13953,13956,14096,14690,15933,16921,16922,19211,
	19990,20205,20208,20326,]]+=0b1000
...
def a17_1975p1_1_cleanup(data):
	...
	f[[2162,2164,2166,2184,2186,2195,2201,2203,2216,2862,3220,3222,5453,6729,10443,11893,11894,12117,13230,13847]]+=0b100

	# Flag extreme outliers in T.
	f[[2169,2184,2186,2193,2196,2205,3220,3222,7283,12117,12713,13847]]+=0b1000
...
def a17_1976p1_1_cleanup(data):
	...
	f[[659,2265,4810,10707,10716,12295,15463,15839,16209,17010,17055,17338,17507,22137,
	22850,22851,22934,23191,27617,28376]]+=0b100

	# Flag extreme outliers in T.
	f[[1314,2103,3233,4810,6978,7337,10712,12295,15463,16209,17055,17388,17490,17507,18165,
	22850,22934,24050]]+=0b1000
...
def a17_1977p1_1_cleanup(data):
	...
	f[[5285,5424,5475,5834,5836,5976,7307,8693,10430,12473,15471,15489,15741,20191,20193,
	20281,24834,24868,24869,24991,25063,26589,27203,28389]]+=0b100

	# Flag extreme outliers in T.
	f[[618,4440,5285,5475,5834,7037,8693,10430,15849,15741,20028,20191,20193,20196,20281,
	24443,24834,24912,24985,24991,25063,26589,27374]]+=0b1000
...
def a17p1_2_cleanup(data):
	...
	dT[0:1]=2*-8-dT[0:1] # Ambiguous. It's possible these initial points don't go so low.
	...
	f[[9,10,11,12,13,14,5523,6156,9834,10489,10607,11114,11341,11492,14051,14494,16030,
	16031,16032,16973,19390,19392]]+=0b100

	# Flag extreme outliers in T.
	f[[5523,6516,9834,11114,11341,11492,12004,14494,16030,16031,18155,19392,20303]]+=0b1000
...
def a17p1_3_cleanup(data):
	...
	f[[2793,6165,7711,9024,10696,10807,14675,14824,16256,16618,19661]]+=0b100000

	# Flag extreme outliers in TC1.

	f[[548,2793,4836,5543,6165,9024,10807,14277,14675,14824,15826,16256,19661,20359]]+=0b1000000

	# Flag extreme outliers in TC2.

	f[[2793,3340,6165,7473,7724,9024,10755,10807,14675,14824,15818,16256,19661,20359]]+=0b10000000

	# Flag extreme outliers in TC3.
	
	f[[2793,4723,4817,6323,6332,9024,9308,10755,10807,11826,12168,12581,14197,14208,14252,
	14675,14690,14824,15721,15735,15818,15826,16256,17148,17319,17353,18593,18651,18669,19335,19625,
	19661,19668,20359,20728]]+=0b100000000

	# Flag extreme outliers in TC4.

	f[[2793,4710,4817,7711,8693,9024,9532,10365,10369,10807,14675,14824,16256,17353,19661,
	20359]]+=0b1000000000
...
def a17p1_4_cleanup(data):
	...
	# Flag extreme outliers in dT.
	f[[]]+=0b100 # None (but noisy)

	# Flag extreme outliers in T.
	f[[]]+=0b1000 # None (but noisy)
...
def a17p2_1_cleanup(data):
	...
	bins=np.array([-2./2**n for n in range(11)])
	...
	# Flag extreme outliers in dT.
	f[[3870,3964,5652,6161,7484,9193,9720,11858,13798,12058,13214,13798,13900,14041,14337,
	4339,15364,15853,16228,16594,16748,18540,18757,19322,20139]]+=0b100
	...
	# Flag extreme outliers in T.
	f[[3964,9695,10693,11858,13798,13814,15364,15875,16594,16748,16749,18540,18757,19231,
	19326,19604,20139]]+=0b1000
...
def a17_1975p2_1_cleanup(data):
	...
	# Flag extreme outliers in dT.
	f[[2174,2184,2192,2208,2733,3025,5427,6244,6320,7258,10304,12102,13197,13216]]+=0b100

	# Flag extreme outliers in T.
	f[[2188,3027,3214,6244,6320,6719,11201,13197,13216]]+=0b1000
...
def a17_1976p2_1_cleanup(data):
	...
	# Flag extreme outliers in dT.
	f[[2416,4816,7055,7062,7268,7275,7462,10722,11879,12093,12300,12370,12373,12495,12621,
	12654,12669,16191,17185,17202,17205,17299,17363,16366,17503,17650,17689,17899,18146,22817,
	22904,25226,26885,27352,29239,29521,28088]]+=0b100

	# Flag extreme outliers in T.
	f[[4816,7062,7275,7418,10729,11879,12093,12370,12495,12669,17185,17202,17205,17363,
	17366,17650,17688,17705,17889,17899,18097,18146,19342,22109,22817,24175,22904,25226,27352,28088]]+=0b1000
...
def a17_1977p2_1_cleanup(data):
	...
	# Flag extreme outliers in dT.
	f[[4812,5197,5221,5467,5835,5836,5984,10409,19608,15399,15464,15716,15729,20064,20243,
	20344,21063,21184,21250,21482,21902,24833,25254,26552,27329,28336,28339,28345]]+=0b100

	# Flag extreme outliers in T.
	f[[5220,5221,5225,5836,10608,15399,15464,15729,19843,20064,20176,20237,20243,21063,
	 21184,21250,21468,21482,24628,24833,24948,25024,25254,26044,28339]]+=0b1000
...
def a17p2_2_cleanup(data):
	...
	f[[6176,6910,7121,7352,8604,9088,9511,9799,9850,11270,11452,12360,14221,14442,14376,
	15468,16852,18644,19855,20017,20243]]+=0b100

	# Flag extreme outliers in T.
	f[[6176,6910,7121,7352,8604,11452,12360,12817,14221,14442,15468,16334,16852,16853,
	18490,18644,19521,20017,20243]]+=0b1000
...
def a17p2_3_cleanup(data):
	...
	f[[13550,15864,16255,16265,16275]]+=0b100000

	# Flag extreme outliers in TC1.

	f[[4721,6160,6677,9222,9326,9561,9967,10697,10749,11261,11465,14027,14150,14704,15864,16255,
	16265,16275,20198]]+=0b1000000

	# Flag extreme outliers in TC2.

	f[[6160,6318,6324,6677,8591,9222,9326,9561,10309,11160,11465,11909,11934,11954,11974,
	12742,14150,14704,15864,16255,16265,16275,16486,16992,18021,18023,18655,19368,19694,20118,
	20119,20198]]+=0b10000000

	# Flag extreme outliers in TC3.
	
	f[[4737,4738,6318,6321,6323,6677,9222,9326,9561,9859,10264,10344,10349,14150,14201,
	14210,14211,14672,14704,14995,15864,16255,16265,16275,20019,20039,20049,20198,
	20689]]+=0b100000000

	# Flag extreme outliers in TC4.

	f[[2105,3226,3990,4720,4738,6314,6315,6318,6677,8692,8694,9222,9326,9561,10309,10344,
	10349,10359,10360,11090,11160,11465,12713,12736,12742,12773,12783,14150,14704,15864,16255,
	16265,16275,16486,18642,18655,19328,19332,19342,19359,20019,20039,20049,20198,20689
	]]+=0b1000000000
